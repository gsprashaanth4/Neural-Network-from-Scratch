...
class Dense:
    ...
    def regularizationLoss(self):
        regularizationLoss = 0 # accumulation of L1 and L2 Regularization
...
class CCEL:
    def forward(self, yCap, y):
        self.yCap = np.clip(yCap, 1e-8, 1 - 1e-8)   # the probs clipped for stability
...
class SoftmaxCCEL:
    ...
    def forward(self, z, y): # last dense output, true values
        self.z = z
        self.y = y
        self.softed = self.af.forward(self.z) # getting the probs from last dense output
        loss = self.lf.forward(self.softed, y)
        return loss
    
    def backward(self):
        self.dLdZ = self.softed - self.y # probs - true values
...
class NNFS:
    def __init__(self, layersD, dropoutRate):
        self.dropoutRate = dropoutRate
        ...
        for i in range(1,len(layersD)-1):
            ...
            self.layers.append(Dropout(self.dropoutRate))
    ...
    def forward(self, inputs):
        ...
        self.prevPushed = inputs
        self.regularizationLoss = 0
        for layer in self.layers:
            self.prevPushed = layer.forward(self.prevPushed)
            if isinstance(layer, Dense):
                self.regularizationLoss += layer.regularizationLoss()
    
    def loss(self, y):
        y = np.atleast_2d(y) # true values
        loss = self.activationLoss.forward(self.prevPushed, y) # last dense output, true values

        preds = np.argmax(self.activationLoss.softed, axis=1)
        labels = np.argmax(y, axis=1)
        accuracy = np.mean(preds == labels)

        return loss + self.regularizationLoss, accuracy

    def backward(self):
        prevDVals = self.activationLoss.backward()
